...
[![TONCoin](https://img.shields.io/badge/donate-TON-blue?logo=TON)](https://github.com/arabezar/xkeen-tg/blob/main/images/tips_TON.jpg)
...
## Примеры вывода команд в Телеграм
  ...
  ```
  Пример вывода команды `/domain + habr`:
  ```text
  Домен добавлен
  Перезапуск прокси...
  ✅ Прокси перезапущен (за 15 сек)
  ```
  Пример вывода команды `/key vless://...` (удачная смена ключа):
  ```text
  Бывший
    url
    vless://...
  ...
  ✅ Прокси перезапущен (за 15 сек)
  ```
  Пример вывода команды `/key vless://...` (неудачная смена ключа):
  ```text
  Бывший
    url
    vless://...
...
Локальный порт для работы socks5 [1080] (Enter - подтвердить):
...
❓ Удалить xkeen-tg? y(N) y
Остановка сервисов бота и вэб-сервера...
 Shutting down tgbotd...              done.
 Checking lighttpd...              alive.
 Shutting down lighttpd...              done.
 Checking tgbotd...              dead.
 Checking lighttpd...              dead.
Удаление конфигурационных файлов...
❓ Удалить настройки бота (переменные и команды)? y(N) y
❓ Удалить команду бота /domain ? y(N) y
...
Removing package netcat from root...
❓ Удалить вэб-сервер lighttpd со всеми установленными модулями? y(N) y
Removing package lighttpd-mod-accesslog from root...
Removing package lighttpd-mod-rewrite from root...
Removing package lighttpd-mod-setenv from root...
Removing package lighttpd-mod-openssl from root...
Removing package lighttpd-mod-proxy from root...
Removing package lighttpd from root...
❓ Удалить инструменты для работы с сертификатами? y(N) y
Removing package openssl-util from root...
✅ Удаление xkeen-tg завершено
root@router:~/xkeentg$
```
</details>
...
* `xkeen-tg` управляется из терминала, вот его список команд, который можно получить запустив его без параметров: `xkeentg`
  ```text
  Основные команды:
    --start       - запуск
    --stop        - останов
...
* Полный список команд управления конфигурацией служб `XKeen` и `xray` роутера с кратким описанием:
  ```text
  ...
  /domain + <domain>       - добавить (раскомментировать) домен в конфигурацию (+ перезагрузка)
