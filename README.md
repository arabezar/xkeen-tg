...
> [!WARNING]
> В связи с тем, что формат конфигурационных файлов `xray` содержит комментарии, не поддерживаемые классическим `json`, обработка файлов (для команд `/domain` и `/key`) производится текстовыми средствами оболочки, очень сильно зависящими от шаблонов поиска/замены, поэтому во-первых, старайтесь придерживаться формата конфигурационных файлов при инсталляции `XKeen`, а во-вторых, при возникновении ошибок, сразу пишите в [Issues](https://github.com/arabezar/xkeen-tg/issues) или [Discussions](https://github.com/arabezar/xkeen-tg/discussions), поправим как можно быстрее
